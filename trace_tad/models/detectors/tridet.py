...
class TriDet(SingleStageDetector):
    ...
    def forward_test(self, inputs, masks, metas=None, infer_cfg=None, **kwargs):
        ...
        points, rpn_reg, rpn_scores, right_pred, left_pred = self.rpn_head.forward_test(x, masks, **kwargs)
        predictions = points, rpn_reg, rpn_scores, right_pred, left_pred
        return predictions

    def post_processing(self, predictions, metas, post_cfg, ext_cls, **kwargs):
        points, rpn_reg, rpn_scores, right_pred, left_pred = predictions

        pre_nms_thresh = getattr(post_cfg, "pre_nms_thresh", 0.01)
        ...
        for i in range(len(metas)):
            ...
            reg = rpn_reg[i].detach().cpu()
            left = left_pred[i].detach().cpu()
            right = right_pred[i].detach().cpu()

            if num_classes == 1:
                segments = self.rpn_head.get_proposals(points, reg.squeeze(0)).detach().cpu()
                ...
                segments = self.rpn_head.get_proposals(points[pt_idxs], reg[cls_idxs, pt_idxs]).detach().cpu()
                scores = pred_prob
                labels = cls_idxs
