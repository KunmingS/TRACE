...
class TriDetBMHead(AnchorFreeHead):
    """TriDet head for use with TemporalDeformableFPN neck.

    Same as TriDetHead (detached boundary branches, IoU-coupled cls loss).
    """
    ...
    def forward_test(self, feat_list, mask_list, **kwargs):
        ...
        for l, (feat, mask) in enumerate(zip(feat_list, mask_list)):
            ...
            # At test time, use detach for efficiency (no backward needed)
            cls_start_feat = feat.detach()
            cls_end_feat = feat.detach()
        ...
        # Same decoding as TriDetHead
        reg_pred = [x.permute(0, 2, 1) for x in reg_pred]
        left, right = self.prepare_out_logits(cls_start_pred, cls_end_pred)
        decoded_offsets = []
        left_pred = []
        right_pred = []
        for i in range(len(left)):
            ...
            for j in range(reg_pred[i].shape[0]):
                res, left_pred_dis, right_pred_dis = self.decode_offset(
                    reg_pred[i][j],
                    left[i][j].permute(1, 0, 2),
                    right[i][j].permute(1, 0, 2),
                    False,
                )
                batch_offsets.append(res)
                batch_left_pred.append(left_pred_dis)
                batch_right_pred.append(right_pred_dis)
            decoded_offsets.append(torch.stack(batch_offsets))
            left_pred.append(torch.stack(batch_left_pred))
            right_pred.append(torch.stack(batch_right_pred))
        reg_pred = torch.cat(decoded_offsets, dim=2)
        left_pred = torch.cat(left_pred, dim=2)
        right_pred = torch.cat(right_pred, dim=2)
        points = torch.cat(points, dim=0)
        scores = torch.cat(cls_pred, dim=-1).permute(0, 2, 1).sigmoid()
        masks = torch.cat(mask_list, dim=1)[..., None]
        scores = scores * masks
        return points, reg_pred, scores, right_pred, left_pred
    ...
    def pad_and_stride(self, cls_level_i, pad_left, num_bins):
        ...
        x = (F.pad(cls_level_i, pad, mode="constant", value=0)).unsqueeze(-1)
        x_size = list(x.size())
        x_size[-1] = num_bins + 1
        x_size[-2] = x_size[-2] - num_bins
    ...
    def prepare_out_logits(self, out_start, out_end):
        ...
        return out_start_logits, out_end_logits
    ...
    def decode_offset(self, out_offsets, pred_left, pred_right, training=True):
        ...
        return torch.cat([decoded_offset_left, decoded_offset_right], dim=-1), pred_left_dis, pred_right_dis
    ...
    def losses(self, cls_pred, reg_pred, mask_list, points, gt_segments, gt_labels,
        ...
        cls_pred_perm = [x.permute(0, 2, 1) for x in cls_pred]
        cls_pred_cat = torch.cat(cls_pred_perm, dim=1)[valid_mask]
        gt_target = gt_cls[valid_mask]

        # optional label smoothing (scalar or per-class list)
        if self._ls_is_per_class:
            ls = gt_target.new_tensor(self.label_smoothing)  # [num_classes]
            gt_target *= 1 - ls
            gt_target += ls / (self.num_classes + 1)
        else:
            gt_target *= 1 - self.label_smoothing
            gt_target += self.label_smoothing / (self.num_classes + 1)

        cls_loss = self.cls_loss(cls_pred_cat, gt_target, reduction="none")

        # 2. regression using IoU/GIoU/DIOU loss (defined on positive samples)
        reg_pred_perm = [x.permute(0, 2, 1) for x in reg_pred]

        # decode the offset
        out_start_logits, out_end_logits = self.prepare_out_logits(out_start, out_end)
        decoded_offsets, _, _ = self.decode_offset(
            reg_pred_perm, out_start_logits, out_end_logits
        )
        decoded_offsets_pos = decoded_offsets[pos_mask]

        # the boundary head predicts per-class, select the correct class
        pred_offsets = decoded_offsets_pos[gt_cls[pos_mask].bool()]
        gt_reg_pos = gt_reg[pos_mask][torch.where(gt_cls[pos_mask])[0]]
        points_cat = torch.cat(points, dim=0).unsqueeze(0).repeat(pos_mask.shape[0], 1, 1)
        points_pos = points_cat[pos_mask][torch.where(gt_cls[pos_mask])[0]]

        # couple the classification loss with iou score
        pred_segments = self.get_proposals(points_pos, pred_offsets)
        gt_segments_decoded = self.get_proposals(points_pos, gt_reg_pos)

        iou_rate = self.iou_loss(pred_segments, gt_segments_decoded, reduction="none")
        # per-class aware threshold for rated_mask
        if self._ls_is_per_class:
            ls_thresh = gt_target.new_tensor(self.label_smoothing) / (self.num_classes + 1)
            rated_mask = gt_target > ls_thresh
        else:
