...
class TriDetHead(AnchorFreeHead):
    ...
    def forward_test(self, feat_list, mask_list, **kwargs):
        ...
        for i in range(len(left)):
            ...
            batch_left_pred = []
            batch_right_pred = []
            for j in range(reg_pred[i].shape[0]): # batch iteration
        ...
        left_pred = torch.cat(left_pred, dim=2)  # [B, C, T, num_bins+1]
        right_pred = torch.cat(right_pred, dim=2)  # [B, C, T, num_bins+1]
    ...
    def losses(self, cls_pred, reg_pred, mask_list, points, gt_segments, gt_labels, out_start, out_end):
        ...
        decoded_offsets, _, _ = self.decode_offset(reg_pred, out_start_logits, out_end_logits)
