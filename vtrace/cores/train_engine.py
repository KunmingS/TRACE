import torch
import tqdm
from vtrace.utils.misc import AverageMeter
...
def train_one_epoch(
    ...
    for iter_idx, data_dict in enumerate(train_loader):
        ...
        if ((iter_idx != 0) and (iter_idx % logging_interval) == 0) or ((iter_idx + 1) == num_iters):
            ...
            block5 = "mem={:.0f}MB".format(peak_mem_mb)
            logger.info("  ".join([block1, block2, "  ".join(block3), block4, block5]))

            # Same cadence as the text log, same numbers — the tracker is a mirror of
            # what already goes to stdout, not a second source of truth.
