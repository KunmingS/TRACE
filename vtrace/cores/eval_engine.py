...
from vtrace.evaluations import build_evaluator
from vtrace.datasets.sliding import SlidingWindowDataset
...
THRESHOLD_SPEC_NAME = "recommended_thresholds.json"


def _is_tuning_subset(subset):
    """Whether ``subset`` is the split thresholds may be tuned on.

    Validation is fair game; anything else (test, in practice) is held out, and
    tuning a cutoff there would inflate the F1 we report from it.
    """
    return subset is not None and "val" in str(subset).lower()


def _export_threshold_spec(work_dir, subset, metrics_dict, logger=None):
    """Persist the validation-tuned cutoffs next to the run's other artifacts.

    A later test evaluation in the same work_dir reads this back, so the held-out
    precision/recall/F1 are reported at a threshold chosen without ever looking
    at test. The same value is what to hand `vtrace predict --threshold`.
    """
    tuning = metrics_dict.get("threshold_tuning")
    if not tuning:
        return None
    # A validation eval that matched no videos (or found no signal) tunes to a
    # degenerate cutoff. Exporting that would silently zero out the later test
    # report, so leave any previous spec alone instead.
    if not tuning.get("global", {}).get("mean_f1", 0.0) > 0.0:
        if logger:
            logger.warning(
                f"Validation eval on '{subset}' produced no usable F1 signal; "
                f"not writing {THRESHOLD_SPEC_NAME}."
            )
        return None
    spec = {
        "subset": subset,
        "global": tuning.get("global", {}).get("threshold"),
        "per_class": {
            label: info.get("threshold")
            for label, info in (tuning.get("per_class") or {}).items()
        },
    }
    path = os.path.join(work_dir, THRESHOLD_SPEC_NAME)
    with open(path, "w") as f:
        json.dump(_jsonify_metrics(spec), f, indent=2)
    if logger:
        logger.info(f"Saved F1-optimal thresholds tuned on '{subset}' to {path}")
    return path


def _apply_saved_threshold_spec(eval_cfg, work_dir, logger=None):
    """Score a held-out split at the validation-tuned threshold, when available.

    Mutates ``eval_cfg`` in place. Config-pinned thresholds always win; if no
    spec file exists the evaluator falls back to tuning on the split it reports,
    and says so in the log. mAP is threshold-free and unaffected either way.
    """
    if eval_cfg.get("type") != "Precision":
        return
    subset = eval_cfg.get("subset")
    if _is_tuning_subset(subset):
        return
    if eval_cfg.get("score_threshold") is not None or eval_cfg.get("per_class_thresholds"):
        return
    ...
    if not os.path.isfile(path):
        if logger:
            logger.info(
                f"No {THRESHOLD_SPEC_NAME} in {work_dir}; precision/recall/F1 on "
                f"'{subset}' will use a threshold tuned on '{subset}' itself "
                f"(optimistic -- run the validation eval first to pin one)."
            )
        return
    ...
    try:
        with open(path) as f:
            spec = json.load(f)
        global_threshold = spec.get("global")
        if global_threshold is None:
            raise ValueError("missing 'global' threshold")
    except Exception as exc:  # noqa: BLE001 - a bad spec must not kill the eval
        if logger:
            logger.warning(f"Ignoring unreadable {path}: {exc}")
        return

    eval_cfg["score_threshold"] = float(global_threshold)
    eval_cfg["per_class_thresholds"] = spec.get("per_class") or None
    eval_cfg["threshold_source"] = f"{path} (tuned on '{spec.get('subset')}')"
    if logger:
        logger.info(
            f"Reporting '{subset}' precision/recall/F1 at the "
            f"'{spec.get('subset')}'-tuned threshold {float(global_threshold):.4f} "
            f"from {path}"
        )
...
def eval_one_epoch(
            ...
            with torch.inference_mode():
                if cfg.inference.load_from_raw_predictions:
                    ...
                    predictions = model.forward_test(
                        ...
                        stream_boxes=data_dict.get("stream_boxes"),
                    )
                    if cfg.inference.save_raw_prediction:
    ...
    if not not_eval:
        ...
        _apply_saved_threshold_spec(eval_cfg, cfg.work_dir, logger)
        evaluator = build_evaluator(dict(prediction_filename=result_eval, **eval_cfg))
        ...
        # Same payload to the experiment tracker. _jsonify_metrics first so numpy
        # scalars/arrays are already plain Python by the time they are flattened.

        # Validation runs export their F1-optimal cutoffs so a later test eval
        # (and `vtrace predict`) can reuse them instead of re-tuning on test.
        if _is_tuning_subset(eval_cfg.get("subset")):
            _export_threshold_spec(cfg.work_dir, eval_cfg.get("subset"), metrics_dict, logger)
