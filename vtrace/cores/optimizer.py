...
def build_optimizer(cfg, model, logger):
    ...
    if hasattr(model, "backbone"):  # if backbone exists
        if model.backbone.freeze_backbone == False:  # not frozen
            assert (
            ...
            # A frozen BackboneWrapper can still own trainable post-pipeline
            # modules, e.g. ConvSpatialPool. Keep explicitly configured custom
            # groups trainable while the actual video backbone forward runs
            # under no_grad().
            if backbone_cfg.get("custom"):
                backbone_optim_groups = get_backbone_optim_groups(backbone_cfg, model, logger)
            else:
    ...
    if "paramwise" in cfg.keys() and cfg["paramwise"]:
        cfg.pop("paramwise")
        det_optim_groups = model.get_optim_groups(cfg)
    else:
