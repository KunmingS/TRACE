...
@PROJECTIONS.register_module(name="TriDetProj")  # deprecated alias (-> SGPPyramidProj)
class SGPPyramidProj(nn.Module):
    """Stride-1 stem + strided SGP branch blocks forming the temporal pyramid.

    Renamed from ``TriDetProj``: the SGP (Scalable-Granularity Perception) blocks are
    genuinely what this builds, but the module is not tied to TriDet as a method. The
    old registry name stays available for configs written before the rename.
    """
    ...
    @property
    def strides(self):
        """Temporal stride of each pyramid level: one stem level at stride 1 followed
        by ``arch[2]`` branch levels each downsampling by ``scale_factor``. Formerly
        supplied by the localization head's PointGenerator (``[1,2,4,8,16,32]`` for the
        default arch=(2,2,5), scale_factor=2)."""
        return [self.scale_factor ** i for i in range(len(self.sgp_win_size))]
...
TriDetProj = SGPPyramidProj  # deprecated alias (renamed -> SGPPyramidProj)
