...
def _detect_decode_source(annotation_path):
    """Where this dataset's frames come from — it dominates loader throughput."""
    try:
        with open(annotation_path, "r", encoding="utf-8") as f:
            database = json.load(f).get("database", {})
    except (OSError, json.JSONDecodeError):
        return "unknown"
    if any(info.get("proxy_video") for info in database.values()):
        return "proxy"
    if any(info.get("cached_video") for info in database.values()):
        return "cached_video"  # legacy per-window clip cache
...
def tune_train_resources(
    ...
    *,
    ...
    log(f"  Decoding from: {_detect_decode_source(annotation_path)}")
    ...
    decode_source = _detect_decode_source(annotation_path)
    if decode_source == "proxy":
        notes.append("Benchmarked against decode proxies; results apply to this prepared dataset.")
    elif decode_source == "source":
        notes.append(
            "Benchmarked against full-resolution source videos; building decode "
            "proxies would change the optimal profile."
        )
    if not successful:
