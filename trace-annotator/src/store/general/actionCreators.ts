...
import {GeneralActionTypes, HomeTab, ProjectData} from './types';
...
export function jumpToFrame(frameIndex: number | null): GeneralActionTypes {
    return {
        type: Action.JUMP_TO_FRAME,
        payload: { frameIndex }
    };
}

export function updateVideoDirectory(videoDirectory: string): GeneralActionTypes {
    return {
        type: Action.UPDATE_VIDEO_DIRECTORY,
        payload: { videoDirectory }
    };
}

export function updateVideoFiles(videoFiles: string[]): GeneralActionTypes {
    return {
...
export function updateHomeTab(homeTab: HomeTab): GeneralActionTypes {
    return {
        type: Action.UPDATE_HOME_TAB,
        payload: { homeTab }
    };
}
