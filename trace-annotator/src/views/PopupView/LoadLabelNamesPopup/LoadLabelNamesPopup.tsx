...
const LoadLabelNamesPopup: React.FC<IProps> = (
    ...
    const loadLabelsFromFile = (file: File) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                if (uniqueNames.length !== names.length) {
                    onFailure(new Error('Label names are not unique'));
                    return;
                }
                const labels: LabelName[] = names.map(name => LabelUtil.createLabelName(name));
                onSuccess(labels);
            } catch (e) {
